...
TG_BTN_EMOJI_ID = "5330237710655306682"  
WA_BTN_EMOJI_ID = "5427009714745517609"  
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "📊 Status":
        ...
        # ⭐️ এখানে ডাইনামিক রিয়েল লোগো বসানো হয়েছে
        status_msg = (
            f"✅ <b>Bot Status: Running Fast! 🚀</b>\n"
            f"{TG_REAL_LOGO} <b>Telegram:</b> {tg_status}\n"
            f"{WA_REAL_LOGO} <b>WhatsApp:</b> {wa_status}\n"
            ...
            f"🖼️ <b>TG Emoji ID:</b> {TG_REAL_LOGO} <code>{TG_BTN_EMOJI_ID}</code>\n"
            f"🖼️ <b>WA Emoji ID:</b> {WA_REAL_LOGO} <code>{WA_BTN_EMOJI_ID}</code>"
        )
        ...
        await update.message.reply_text(f"{TG_REAL_LOGO} <b>Telegram is now {'ON' if TG_ENABLED else 'OFF'}!</b>", reply_markup=get_main_keyboard(), parse_mode="HTML")
        return ConversationHandler.END

    elif text == "🔘 WA: ON/OFF":
        WA_ENABLED = not WA_ENABLED
        await update.message.reply_text(f"{WA_REAL_LOGO} <b>WhatsApp is now {'ON' if WA_ENABLED else 'OFF'}!</b>", reply_markup=get_main_keyboard(), parse_mode="HTML")
        return ConversationHandler.END
...
async def update_tg_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    TG_REAL_LOGO = f'<tg-emoji emoji-id="{TG_BTN_EMOJI_ID}">✈️</tg-emoji>'
    # ⭐️ আইডি সেট করার সাথে সাথেই এখানে রিয়েল লোগো দেখাবে
    await update.message.reply_text(f"✅ <b>TG Emoji ID Updated:</b> {TG_REAL_LOGO} <code>{TG_BTN_EMOJI_ID}</code>", reply_markup=get_main_keyboard(), parse_mode="HTML")
    return ConversationHandler.END
...
async def update_wa_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    WA_REAL_LOGO = f'<tg-emoji emoji-id="{WA_BTN_EMOJI_ID}">🟢</tg-emoji>'
    # ⭐️ আইডি সেট করার সাথে সাথেই এখানে রিয়েল লোগো দেখাবে
    await update.message.reply_text(f"✅ <b>WA Emoji ID Updated:</b> {WA_REAL_LOGO} <code>{WA_BTN_EMOJI_ID}</code>", reply_markup=get_main_keyboard(), parse_mode="HTML")
    return ConversationHandler.END
...
if __name__ == '__main__':
    if not BOT_TOKEN:
        print("❌ Error: BOT_TOKEN is missing! Please set it in Railway Variables.")
    else:
        ...
        print("✅ Bot is successfully running from GitHub...")
